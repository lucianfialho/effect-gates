...
```bash
pnpm add @gatesai/runtime @gatesai/providers
```
...
| Package | Description |
|---|---|
| [`@gatesai/runtime`](packages/runtime) | Agent, Harness, SessionHistory, Compaction, Events |
| [`@gatesai/providers`](packages/providers) | Anthropic, MiniMax, OpenAI — with tool calling |
| [`@gatesai/skills`](packages/skills) | YAML state machines, Tasks, Connectors, Interpolation |
| [`@gatesai/sandbox`](packages/sandbox) | Local execution with path traversal and credential isolation |
| [`@gatesai/core`](packages/gates) | Atomic primitives: bash safety, dedup, file metadata |
| [`@gatesai/harness-ui`](packages/harness-ui) | Terminal UI — chat, sidebar, skill visualization |
| [`@gatesai/cli`](packages/cli) | CLI: `gates run`, `chat`, `resume`, `dev`, `skill` |
...
```typescript
import { makeAgent } from "@gatesai/runtime";
import { makeMiniMaxProvider } from "@gatesai/providers";
...
- `@gatesai/harness-ui` sidebar — split layout (36 cols) with `Ctrl+S` toggle, auto-opens on skill completion
...
**Sessions list** (`@gatesai/harness-ui`)
